...
rem インストールディレクトリ
set finalDir=%sourceDir%\Final\v%vcVersion%\lmdb

rem ビルドディレクトリ確認
rem if not exist "%buildDir%" (
	rem Configuration実行
	call %batchPath%lmdb_Configuration.bat %1 %2
rem )
...
if /%buildType%==/install (
	...
	rem バージョン番号ファイル追加
	type nul > %finalDir%\lmdb_%lmdbVersion%
)
...
rem binディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="lmdb*.dll;lmdb*.pdb" /exclude="*\" "%buildDir%" /to="%finalDir%\bin\%platformName%"

rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="lmdb*.lib" /exclude="*\" "%buildDir%" /to="%finalDir%\lib\%platformName%"
...
rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="lmdb*.lib" /exclude="*\" "%buildDir%" /to="%finalDir%\lib\%platformName%\static"
