...
if(MSVC AND MKL_USE_DLL)
    set(MKL_POSTFIX "_dll")
else()
    set(MKL_POSTFIX "")
endif()
...
if(${MKL_VERSION_STR} VERSION_GREATER "11.3.0" OR ${MKL_VERSION_STR} VERSION_EQUAL "11.3.0")
    ...
    if(MKL_WITH_TBB)
        if(MKL_USE_TBB_PREVIEW)
            list(APPEND mkl_lib_list mkl_tbb_thread${MKL_POSTFIX} tbb_preview)
        else()
            list(APPEND mkl_lib_list mkl_tbb_thread${MKL_POSTFIX} tbb)
        endif()
    elseif(MKL_WITH_OPENMP)
        if(MSVC)
            list(APPEND mkl_lib_list mkl_intel_thread${MKL_POSTFIX} libiomp5md)
        else()
            list(APPEND mkl_lib_list mkl_gnu_thread)
        endif()
    else()
        list(APPEND mkl_lib_list mkl_sequential${MKL_POSTFIX})
    endif()

    list(APPEND mkl_lib_list mkl_core${MKL_POSTFIX})
else()
