...
rem インストールディレクトリ
set finalDir=%sourceDir%\Final\v%vcVersion%\LevelDB

rem ビルドディレクトリ確認
rem if not exist "%buildDir%" (
	rem Configuration実行
	call %batchPath%leveldb_Configuration.bat %1 %2
rem )
...
if /%buildType%==/install (
	...
	rem バージョン番号ファイル追加
	type nul > %finalDir%\LevelDB_%leveldbVersion%
)
...
rem binディレクトリコピー
rem %fastcopyExe% %fastcopyMode% /cmd=diff /include="leveldb*.dll;leveldb*.pdb" /exclude="leveldbutil*;leveldb_*;*\" "%buildDir%" /to="%finalDir%\bin\%platformName%"

rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="leveldb*.lib" /exclude="leveldb_*;*\" "%buildDir%" /to="%finalDir%\lib\%platformName%"
...
rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="leveldb*.lib" /exclude="leveldb_*;*\" "%buildDir%" /to="%finalDir%\lib\%platformName%\static"
