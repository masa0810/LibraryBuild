...
rem インストールディレクトリ
set finalDir=%sourceDir%\Final\v%vcVersion%\Snappy

rem ビルドディレクトリ確認
rem if not exist "%buildDir%" (
	rem Configuration実行
	call %batchPath%Snappy_Configuration.bat %1 %2
rem )
...
if /%buildType%==/install (
	...
	rem バージョン番号ファイル追加
	type nul > %finalDir%\Snappy_%snappyVersion%
)
...
rem binディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="snappy*.dll;snappy*.pdb" /exclude="snappy-*;*\" "%buildDir%" /to="%finalDir%\bin\%platformName%"

rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="snappy*.lib" /exclude="snappy_*;*\" "%buildDir%" /to="%finalDir%\lib\%platformName%"
...
rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="snappy_static*.lib" /exclude="*\" "%buildDir%" /to="%finalDir%\lib\%platformName%"
