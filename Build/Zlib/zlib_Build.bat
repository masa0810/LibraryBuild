...
rem インストールディレクトリ
set finalDir=%sourceDir%\Final\v%vcVersion%\zlib

rem ビルドディレクトリ確認
rem if not exist "%buildDir%" (
	rem Configuration実行
	call %batchPath%zlib_Configuration.bat %1
rem )
...
if /%buildType%==/install (
	...
	rem binディレクトリコピー
	%fastcopyExe% %fastcopyMode% /cmd=diff /include="zlib*.dll;zlib*.pdb" /exclude="*\" "%buildDir%" /to="%finalDir%\bin\%platformName%"

	rem libディレクトリコピー
	%fastcopyExe% %fastcopyMode% /cmd=diff /include="*zlib*.lib" /exclude="*\" "%buildDir%" /to="%finalDir%\lib\%platformName%"

	rem バージョン番号ファイル追加
	type nul > %finalDir%\zlib_%zlibVersion%
)
