...
rem インストールディレクトリ
set finalDir=%sourceDir%\Final\v%vcVersion%\ProtoBuf

rem ビルドディレクトリ確認
rem if not exist "%buildDir%" (
	rem Configuration実行
	call %batchPath%ProtoBuf_Configuration.bat %1 %2
rem )
...
if /%buildType%==/install (
	...
	rem バージョン番号ファイル追加
	type nul > %finalDir%\ProtoBuf_%protoBufVersion%
)
...
rem binディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="libproto*.dll;libproto*.pdb" /exclude="*\" "%buildDir%" /to="%finalDir%\bin\%platformName%"

rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="libproto*.lib" /exclude="*\" "%buildDir%" /to="%finalDir%\lib\%platformName%"
...
rem libディレクトリコピー
%fastcopyExe% %fastcopyMode% /cmd=diff /include="libproto*.lib;libproto*.pdb" /exclude="*\" "%buildDir%" /to="%finalDir%\lib\%platformName%\static"

if /%configType%==/Release (
	rem binコピー
	%fastcopyExe% %fastcopyMode% /cmd=diff "protoc.exe" /to="%finalDir%\bin\%platformName%"
)
